...
bool ModelClass::LoadTexture( WCHAR* filename)
{
	...
	result = m_Texture->Initialize(D3D::GetDevice(), filename);
	if (!result) { return false; }
...
bool ModelClass::LoadMultiTexture( WCHAR* filename, WCHAR* filename2)
{
	...
	result = m_Texture->InitializeMulti(D3D::GetDevice(), filename, filename2);
	if (!result) { return false; }
...
bool ModelClass::LoadTripleTexture( WCHAR* filename, WCHAR* filename2, WCHAR* filename3)
{
	...
	result = m_Texture->InitializeTriple(D3D::GetDevice(), filename, filename2, filename3);
	if (!result) { return false; }
